...
export default function GameBoard({
  ...
  useEffect(() => {
    ...
    let filtered = songs.filter((song) => {
      ...
      try {
        const masterLevel = Number.parseFloat(
          song.level_master.replace("+", ".7")
        );
        const minMasterLevel = Number.parseFloat(
          settings.masterLevelRange.min.replace("+", ".7")
        );
        const maxMasterLevel = Number.parseFloat(
          settings.masterLevelRange.max.replace("+", ".7")
        );
      ...
      result: {
        ...
        masterLevel: {
          ...
          direction: compareValues(
            song.level_master,
            gameState.targetSong.level_master
          ),
          close: isClose(song.level_master, gameState.targetSong.level_master),
        },
        masterDesigner:
          song.charts.master.designer ===
          gameState.targetSong.charts.master.designer,
        remasterLevel: {
          value: song.level_remaster === gameState.targetSong.level_remaster,
          direction: compareValues(
            song.level_remaster,
            gameState.targetSong.level_remaster
          ),
          close: isClose(
            song.level_remaster,
            gameState.targetSong.level_remaster
        ...
        remasterDesigner:
          song.charts.remaster?.designer ===
          gameState.targetSong.charts.remaster?.designer,
        version: {
