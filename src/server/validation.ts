...
const avatarUrlSchema = z.string().url().max(500).optional()
...
export const socketSchemas = {
  ...
  joinRoom: z.object({ roomId: roomIdSchema, nickname: nicknameSchema, avatarUrl: avatarUrlSchema }).strict(),
  reconnectSession: z.object({ sessionToken: z.string().min(16).max(200) }).strict(),
  joinRandomRoom: z.object({ nickname: nicknameSchema, avatarUrl: avatarUrlSchema }).strict(),
